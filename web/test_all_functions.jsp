...
<body>
    <div class="container">
        ...
        <p>本页面用于测试 com.ServiceLayer.java 中的所有功能接口</p>
                            ...
                            <table>
                                ...
                                <% for (Model.Product product : products) { %>
                                    <tr>
                                        <td><%=product.id%></td>
                                        <td><%=product.name%></td>
                                        <td><%=ServiceLayer.formatPrice(product.price)%></td>
                                        <td><%=product.stock%></td>
                                        <td><%=product.description != null ? product.description : "无描述"%></td>
                                    </tr>
                                <% } %>
                            </table>
                        <% } else { %>
            ...
            %>
                    <div class="result info">
                        <% if (product != null) { %>
                            ...
                            <p><strong>价格:</strong> <%=ServiceLayer.formatPrice(product.price)%></p>
                            <p><strong>库存:</strong> <%=product.stock%></p>
                            <p><strong>描述:</strong> <%=product.description != null ? product.description : "无描述"%></p>
                        <% } else { %>
                            <p>商品不存在</p>
                        <% } %>
                    </div>
                            ...
                            <table>
                                ...
                                <% for (Model.Order order : userOrders) { %>
                                    <tr>
                                        <td><%=order.id%></td>
                                        <td><%=order.userId%></td>
                                        <td><%=ServiceLayer.formatDateTime(order.orderDate)%></td>
                                        <td><%=order.status%></td>
                                        <td><%=ServiceLayer.formatPrice(order.total)%></td>
                                    </tr>
                                <% } %>
                            </table>
                        <% } else { %>
                            ...
                            <table>
                                ...
                                <% for (Model.UserProduct up : userProducts) { %>
                                    <tr>
                                        ...
                                        <td><%=up.sn%></td>
                                        <td><%=up.afterSaleStatus != null ? up.afterSaleStatus : "正常"%></td>
                                    </tr>
                                <% } %>
                            </table>
                        <% } else { %>
        ...
        <div id="utility-section" class="section">
            ...
            <div class="result info">
                <h4>价格格式化演示:</h4>
                <p>原始价格: 2999.0 → 格式化后: <%=ServiceLayer.formatPrice(2999.0)%></p>
                <p>原始价格: 1234.56 → 格式化后: <%=ServiceLayer.formatPrice(1234.56)%></p>
                
                <h4>时间格式化演示:</h4>
                <p>当前时间: <%=ServiceLayer.formatDateTime(new java.sql.Timestamp(System.currentTimeMillis()))%></p>
                
                <h4>字符串检查演示:</h4>
                <p>空字符串检查: <%=ServiceLayer.isEmpty("")%></p>
                <p>null检查: <%=ServiceLayer.isEmpty(null)%></p>
                <p>正常字符串检查: <%=ServiceLayer.isEmpty("hello")%></p>
                
                <h4>安全转换演示:</h4>
                <p>字符串"123"转整数: <%=ServiceLayer.safeParseInt("123", 0)%></p>
                <p>字符串"abc"转整数(默认值0): <%=ServiceLayer.safeParseInt("abc", 0)%></p>
                <p>字符串"99.99"转浮点数: <%=ServiceLayer.safeParseDouble("99.99", 0.0)%></p>
                <p>字符串"xyz"转浮点数(默认值0.0): <%=ServiceLayer.safeParseDouble("xyz", 0.0)%></p>
            </div>
        </div>
        ...
        <div class="section">
            <h2>7. 当前会话信息</h2>
            <div class="result info">
                <p><strong>当前用户:</strong> <%=session.getAttribute("username") != null ? session.getAttribute("username") : "未登录"%></p>
                <p><strong>当前管理员:</strong> <%=session.getAttribute("admin") != null ? session.getAttribute("admin") : "未登录"%></p>
                <p><strong>用户ID:</strong> <%=session.getAttribute("userId") != null ? session.getAttribute("userId") : "无"%></p>
            </div>
        </div>
