...
public class ServiceLayer {
     ...
     * 使用方法：
     * <%
     *     boolean loginSuccess = ServiceLayer.userLogin("用户名", "密码");
     *     if (loginSuccess) {
     *         // 登录成功，设置session
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.userRegister("新用户名", "密码");
     ...
     * 使用方法：
     * <%
     *     boolean adminLogin = ServiceLayer.adminLogin("管理员用户名", "密码");
     *     if (adminLogin) {
     ...
     *     Model.Product product = ServiceLayer.getProductById(productId);
     *     if (product != null) {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.addProduct("小米手机", 2999.0, 100, "高性能智能手机");
     *     if ("success".equals(result)) {
     *         out.println("商品添加成功");
     *     } else {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.updateProduct(1, "小米手机Pro", 3299.0, 50, "升级版智能手机");
     *     if ("success".equals(result)) {
     *         out.println("商品更新成功");
     *     } else {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.deleteProduct(1);
     *     if ("success".equals(result)) {
     *         out.println("商品删除成功");
     *     } else {
     ...
     *     List<Model.Order> orders = ServiceLayer.getUserOrders(userId);
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.updateOrderStatus(orderId, "已发货");
     *     if ("success".equals(result)) {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.bindUserProduct(userId, productId, "SN123456789");
     *     if ("success".equals(result)) {
     *         out.println("商品绑定成功");
     *     } else {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.applyAfterSale(userProductId);
     *     if ("success".equals(result)) {
     ...
     * 使用方法：
     * <%
     *     String result = ServiceLayer.updateAfterSaleStatus(userProductId, "已处理");
     *     if ("success".equals(result)) {
     ...
     * 使用方法：
     * <%
     *     String formattedPrice = ServiceLayer.formatPrice(2999.0);
     ...
     * 使用方法：
     * <%
     *     String formattedTime = ServiceLayer.formatDateTime(order.orderDate);
